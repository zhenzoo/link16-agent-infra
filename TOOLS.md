...
| `feishu/send_feishu_msg.py` ⭐ | 主动发**纯文字 + @人/@bot**（agent↔agent 喊话原语）· **`--to-agent <名> --wait <秒>` = a2a-reply-wait**（守望对端文字回复·跳启动卡·收全这轮·ARCH-140 §5） | `python feishu/send_feishu_msg.py --bot X --to-agent Y --text "..." --wait 120` |
