...
| `feishu/register_feishu_app.py` | **一键建飞书 bot**（profile doctor + 扫码 OAuth + 预置 40+ 权限 + WS + 本机 roster upsert）· 末步打印开全权限链；页面已建应用但 CLI 回传中断时可续接 | `python feishu/register_feishu_app.py --name X --bot wsN --profile cxp [--app-id cli_...]` |
