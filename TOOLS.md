...
| `feishu/feishu_docs.py` | `send --doc`底层：过在线策略闸，Markdown/TXT复用docio正式写入并独立逐格核验原生表格；取消24格和竖线文字降级，写后失败保留文档ID。HTML/Office走import，禁止自动发附件，见SOP-140 | 全局 on：`python feishu/feishu_bridge.py send --bot X --doc <file>`；当前轮明确要求在线稿：追加 `--explicit-online` |
