...
| `feishu/profile_bootstrap.py` | **同事自助 profile/skill/hooks 入口**：初始化、迁移或追加本机 `agent-profiles.local.json`；为所选 profile 建隔离 home、动态 Git Bash/PowerShell wrapper 并安装 repo-owned `feishu` skill；Codex profile 同时无损合并三类 bridge hooks；wrapper 与治理 doctor 共用 `profile_wrappers.py`，不再各存一份模板 | `--init-registry` / `--migrate-registry` / `--register-profile NAME` 均先预览再 `--apply`；最后 `--doctor` |
...
| `feishu/session_work.py` ⭐ | **每只 bot 的「工作行」**：**每张卡片的带色标题条** = 项目 pill + `📌 <项目> · <对象+动作+交付结果>`（颜色跟进度：🔴 红 / 全 ✅ 绿 / 其余蓝），正文首行 `<Stage 链带状态>`，标题同时进卡片摘要（会话列表预览行），十几个 bot 并排时不点开也认得出它在做哪个项目、做到哪。agent 接新任务 / Stage 切换 / 旧任务做完时自己 `set`（task 点名具体对象与动作，不写抽象类别；progress 有 PLAN 按 PLAN 的 Stage 写）；没写就兜底 = cwd 目录名 + Claude `ai-title`；`/new` `/clear` `/cd` 自动清。身份闸=只能写自己的。机制见 ARCH-110 §2.6.1 | `python feishu/session_work.py set --project TC101P --task "给 TC101P 的 PRD 补可行性章节，交付改好的 PRD.md" --progress "找素材 ✅ → 写帖子 🔄 → 得出结论 ⏳"` · `show` · `clear` |
