...
| `feishu/reset_bot_identity.py` | **同名重建一只 bot 时，清掉上一个飞书应用留下的身份状态**（`owner` / `session` / `turn-route` / `delivery-state` / `answer-state` / `pending` / `watchdog-handoff`），**保住** inbound/outbox/outbound/receipts 这批按名字存的收发记录 + `outbox-hwm`/`stop-cursor` 两个防重发游标。默认 dry-run；`--apply` 也只是 move 进 `_state/_bot-reset-archive/`，不真删。`--refresh-openid` 补 `register_feishu_app` 幂等跳过留下的坑（重建后 `agent-registry.json` 的 open_id 不会自动刷新）。⚠️ `open_id` 是 per-app 的，不清 owner 文件会 `230013` → 刷群 | `python feishu/reset_bot_identity.py --bot X`；`--apply`；`--refresh-openid --apply` |
