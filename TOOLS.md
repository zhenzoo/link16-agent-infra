...
| **`/handoff`（飞书里发）** ⭐ | **`/close` 的进阶版：换一个全新 context，但让它先读懂历史再跟你对齐**。关掉当前会话 → **账号和目录都不变** → 起一个全新会话 → 自动注入 prompt 让它：读上一轮 transcript（先读尾部·禁止通读）+ **重点看最后几轮那份还没定的方案** + **调研 code base**（不只读聊天记录）→ 汇报「原任务/已完成/停在哪/哪些还没定」→ **停下等你提新需求**。启动按 runtime 使用独立窗口；失败 pane 关闭前会保存有界现场，随后发 `/screen` 仍能查看。 | 在飞书里 @ 该 bot 发 `/handoff`（或 `/交接`） |
