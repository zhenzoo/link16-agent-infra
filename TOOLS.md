...
| `feishu/feishu_docs.py` | 本地 md/HTML → 飞书在线文档（`send --doc` 底层）：有权限走 upload/import；缺 `drive:drive` 的 Markdown/TXT 自动走当前 bot 的原生 docx 链；两条都失败后才允许按已确认的本机策略发附件，**不跨 bot 代发**。链接访问者仍须登录飞书；CLI/receipt 报真实源字符/字节并做 p2a URL 对账 | `python feishu/feishu_bridge.py send --bot X --doc <file>`；正文进聊天框必须显式 `--file-as-text <file>` |
