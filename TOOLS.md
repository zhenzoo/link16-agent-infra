...
| `feishu/artifact_delivery.py` | **本机全局产物交付策略 SSOT**：只决定是否允许创建飞书在线副本；默认 off，所有 Link16 profile/bot 共读。绝对路径始终显示、附件仍需明确授权、GUI 打开仍由 `$open-local` 决定；`send --doc` / 在线媒体在网络前执行失败闸；**`receipt` 渲染 SPEC-210 固定三行产物回执**（标题行 / URL 行 / 本机绝对路径行，缺行括号说明），`send --doc`、媒体卡、drainer 对账块共用 | `status [--json]`；`set-online on|off`；`decide --explicit auto|online|local [--json]`；`receipt --title <标题> [--url <URL>] [--path <文件>]... [--reason <无URL原因>] [--json]` |
...
| `feishu/agent_quota.py` ⭐ | **查各账号还剩多少额度（实时·唯一真源）**：Claude 走 `api.anthropic.com/api/oauth/usage`，Codex 走 `chatgpt.com/backend-api/codex/usage`；两条线路自动择通。**绝不读本地缓存**；只有“够用/紧张”进入 `pick`，“满/问不到”均 fail closed | `python feishu/agent_quota.py`（表）· `--json` · `pick --exclude <profile> --prefer-runtime claude`（该切哪个号） |
...
| `feishu/profile_bootstrap.py` | **同事自助 profile/skill/hooks 入口**：初始化、迁移或追加本机 `agent-profiles.local.json`；为所选 profile 建隔离 home、动态 Git Bash/PowerShell wrapper，并安装 repo-owned `feishu` 与显式触发的 `feishu-workline` skills；Codex/Kimi profile 无损合并 bridge hooks；wrapper 与治理 doctor 共用 `profile_wrappers.py` | `--init-registry` / `--migrate-registry` / `--register-profile NAME` 均先预览再 `--apply`；最后 `--doctor` |
| `.agents/skills/link16-init/SKILL.md` ⭐ | **首次安装一站式剧本（PLAN-1000）**：用户说「开始部署 Link16 / 初始化 / 把我的记忆导进来」就按它走——装依赖 → profile 登录 → 盘点已有记忆与活跃项目 → 勾选导入 → 按「机器代号-项目简称」建 2～3 只 bot → 验收；完成判据 = 私聊 bot 问「我在做什么项目、我有什么偏好」答得出来 | 在 Claude/Codex 桌面客户端或 CLI 里说「开始部署 Link16」；Claude Code 项目级薄壳在 `.claude/skills/link16-init/` |
| `feishu/context_scan.py` | **只读盘点本机已有 AI context**：`~/.claude`（CLAUDE.md/memory/skills/会话 cwd）、Claude 桌面版 Cowork 记忆（exe 与 MSIX 两种落点）、`~/.codex`、ChatGPT 桌面版（只报需导出）、下载目录里的官方导出包；再扫近 7 天活跃项目（git 提交/改动文件/Claude 会话三路打分）并建议 `<机器代号>-<项目简称>` bot 名（代号推不出用 hostname，不问）。只数不读、永不列凭据 | `python feishu/context_scan.py [--out feishu/_state/context-scan.json] [--roots DIR] [--days 7] [--top 3] [--json]` |
| `feishu/context_import.py` | **勾选式导入到所选 profile home**：CLAUDE.md/AGENTS.md 以带 `imported-from` 标记的独立段追加（同源同 hash 幂等）、记忆笔记补 frontmatter 进 `<home>/memory/` + MEMORY.md 索引、项目记忆按 slug 对拷、skills/commands 复制（feishu 跳过）、settings 只合并白名单偏好键、导出包 memories/projects 转记忆、conversations 抽近 90 天交 `claude -p` 蒸馏成 `imported-taste-*.md`（默认开）；永不复制 `.claude.json`/`.credentials.json`/`auth.json`/`.env`/sessions；每次写 receipt，`--rollback` 一键撤回 | `python feishu/context_import.py --profile <名> [--scan r.json] [--only id,id] [--skip id] [--no-distill]` 预览；加 `--apply` 才写；`--rollback --receipt <file>` |
| `feishu/windows_bootstrap.py` | **新机双层人话清单**：软件层检测 Git/GH/Python/Node/wmux/Claude/Codex/Kimi；三种 CLI 默认走官方原生安装器。Link16 层展示 profile/skill/hooks/名册/凭据/桥/cron/watchdog/注册监督/历史账本。只使用“将安装/已存在跳过/已验证/需要登录/需要人工确认”等用户状态；gstack 默认关闭 | `python feishu/windows_bootstrap.py`；确认后 `--apply --yes [--skip claude,codex,kimi]`（逗号列出要跳过的项） |
...
| `feishu/codex_app_server_probe.py` · `codex_app_server_worker.py` | Codex typed-event 只读探针；app-server worker（官方 TUI `--remote` + observer）= **所有 Codex profile 会话的默认投递路**，typed commentary/tool 写进度、typed final 写答案，均不依赖账号 hooks | `python feishu/codex_app_server_probe.py`；生产由桥自动起（`agent_runtime.uses_app_server`·名册写 `codex_transport: cli-legacy` 才回退老路） |
...
| `feishu/rename_bot.py` · `feishu/bot_names.py` | **已有智能体改名**：专属后台链接、线上名称回读、运行/舰队名册显示名和历史别名同步；固定内部代号、凭据、会话与 cron，反重名/陈旧计划，写后回读及可恢复回执；通用名称解析由 bot_names 复用 | `plan --bot <原名/别名> --name <新名> --out feishu/_state/bot-renames/<操作>.json` → `apply --plan <文件>` → `verify --bot <新名>`；人工操作可 `watch --plan <文件> --background`；详见 `docs/SOP-125-bot-rename.md` |
...
| `feishu/docio_cli.py` ⭐ | **飞书文档 IO 唯一入口**：`inspect` 解析链接与可达性 · `read --into` 一条命令读全（正文/表格/图片/附件/评论 + manifest）· `write --patch` 默认 dry-run、`--apply` 后逐格回读核验 · `share` 把协作群挂为协作者 · `transfer-owner` 把 bot 建的文档所有权交给主人（bot 保留 full_access·其余协作者与分享设置一律不动·默认 dry-run·回读所有者才算成）· `coverage` 机械判定是否读全 · `doctor` 身份/基线/资源三层体检 · `profiles` 一致性校验。**身份按当前 bot 从 `.env` 取、用完即弃；解析不出身份直接失败，绝不借别的 bot 的壳** | `python feishu/docio_cli.py read <url> --into <dir>`；`--bot X` 显式指定身份；`share <url> --apply`；`transfer-owner <url> --apply`；`coverage <dir>/manifest.json` |
...
| `feishu/feishu_docs.py` | `send --doc` 底层：Markdown/TXT先编译布局，非PRD表格转原生字段列表；写后完整回读正文、链接、样式及结构，失败禁止纯文本降级/import绕闸。HTML/Office走import，另按对应流程验收。网络前先过全局策略闸，失败不发附件 | 全局 on：`python feishu/feishu_bridge.py send --bot X --doc <file>`；当前轮明确要求在线稿：追加 `--explicit-online` |
| `feishu/doc_structure.py`、`feishu/doc_xml_structure.py` | 在线格式机械闸：原生Markdown创建编译与回读契约；docio XML更新绑定revision并全文回读，局部替换须提供expected_document。只验证声明的文字/结构/资源范围，不代替手机观感与视频播放验收 | 由 `send --doc` / `docio_cli.py write --patch ... --apply` 强制调用；反例测试 `test_feishu_doc_structure.py` / `test_docio_xml_structure.py` |
| `feishu/send_feishu_msg.py` ⭐ | 主动发**纯文字 + @人/@bot**；发送前检查 active turn，目标等于本轮自动回址时在网络前拒绝，防“手动一次 + 自动一次”。成功写 unified outbound ledger；真正额外通知才加 `--proactive`。发 peer 仍是 ARCH-140 的唯一主动路；收件 bot 在另一个飞书租户时自动改走对方共享群的自定义机器人 webhook（ARCH-140 §8） | `python feishu/send_feishu_msg.py --bot X --to-agent Y --text "..." [--proactive]` |
| `feishu/session_work.py` ⭐ | **每只 bot 的「工作行」与逐 turn 回执闸**：入站先建 `pending`，机械触发 repo-owned `feishu-workline` 让 LM 选择 `keep / replace / progress`；普通工作卡只在 `ready` 后出站。首个 Stop 缺回执会续跑一次，第二次仍缺则改成红色故障标题后放行，既不静默缺标题也不无限循环。语义字段只有 `project / task / progress`，身份、route、turn key、revision、状态、配色和摘要均由桥填写；旧 `set` 保持兼容，`/new` `/clear` `/cd` 自动清。机制见 ARCH-110 §2.6.1 | `python feishu/session_work.py decide --turn-key <key> --action replace --project TC101P --task "给 TC101P 的 PRD 补可行性章节，交付改好的 PRD.md" --progress "找素材 ✅ → 写帖子 🔄 → 得出结论 ⏳"` · `set` · `show` · `clear` |
| `feishu/send_feishu_file.py` ⭐ | 发**文件本体**附件（仅用户本轮明确要“附件/原文件”时；默认交付走在线文档） | `python feishu/send_feishu_file.py --bot X --to oc_群 --file <f>` |
| `feishu/send_feishu_voice.py` ⭐ | 发**可拖进度条语音**（带 duration；仅用户本轮明确要“语音条”时，其余音频走 `send_feishu_media.py`） | `python feishu/send_feishu_voice.py --bot X --audio <a> --text "说明"` |
| `feishu/send_feishu_media.py` ⭐ | 新建或往已有文档插入原生图片／音视频，超过20MiB自动分片；同一入口可逐项验证真实页面，失败不发信；已有文档正文和权限保持，见 SOP-141 | `python feishu/send_feishu_media.py --bot X --media <m> --publish-only --receipt <新的JSON> --verify-out <新目录>`；已有文档加 `--document <docx URL/token>`，位置加 `--parent-block <id> --index <0起>`；本轮在线覆盖加 `--explicit-online` |
| `feishu/verify_media_doc.py` | 在真实飞书文档点击播放、跳转并记录下载事件；手机浏览器模拟不冒充真机App实测 | `python feishu/verify_media_doc.py --url <url> --block-id <id> --file-token <token> --media <文件> --out <新目录> --mobile` |
| `feishu/feishu_rest.py` | 飞书 REST 原语（api/tenant_token/send_msg · 纯标库绕代理） | （feishu_docs/media/voice 内部用） |
| `feishu/agent-profiles.local.json` · `feishu/agent_profile_cli.py` · `feishu/agent_runtime.py` | 本机 effective profile registry（gitignored）+ 公共 launcher：主 session/独立 wmux worker 都只传 `LINK16_AGENT_PROFILE`，由 registry 派生 runtime/home/driver；`session_search.preferred_profiles` 是历史 Session 首轮搜索集合的唯一真源 | `python feishu/agent_profile_cli.py list`；`platforms`（Link16 支持的智能体平台 × 本机安装与账号）；`doctor --profile <profile>`；`run --profile <profile> --cwd <repo>` |
...
- `$agent-profile-governance`：维护者自己的完整用户规则/skills 治理；其 `scripts/profile_lifecycle.py` 是 Claude/Codex/Kimi 账号 create/remove/status 的确定性入口，完整合同见用户级 SOP-010；只有出现“账号可用但禁止自动选号”的真实需求时，才增加窄范围 `auto_failover: false`。
