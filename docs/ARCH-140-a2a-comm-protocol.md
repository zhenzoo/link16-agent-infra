...
**路由**（`send_feishu_msg --to-agent`）：名册里发件 bot 与收件 bot 都登记了 `tenant_key` 且不同 → 取收件方租户的 `webhook_env`，从本机 `.env` 读地址后经 webhook 原样投递（文字 + 真 @ 标签 + 发件人戳，不另加前缀），路由记为 `a2a-webhook`；任一边没登记 `tenant_key` 或相同 → 原来的共享群路径不变。收件租户没登记 webhook、或本机 `.env` 缺地址 → 当场报错，不退回别的群。
