...
连不上飞书（DNS、网络、握手失败）时桥不设超时、不自己 stop 重开，交给飞书 SDK 无限重连：首次连接和运行中断线都一样，间隔为 SDK 默认 120 秒加 0–30 秒随机抖动，连上后改用服务端下发的间隔。桥只订阅 SDK 的 `reconnecting` / `reconnected` 事件：断开超过 60 秒用 REST 纯文字给主人发一条通知，恢复后再发一条，中间不重复。自己 stop 再重开同一个 channel 会让 SDK 旧线程留下第二条长连接（09-24 baseball-6 实证），所以不这样做。主循环其他未预期异常立刻 `os._exit(1)` 并记 `failed`，不再让 SDK 线程把进程吊成收不到消息的僵尸；看门狗 R4 每轮对本机名册里没有进程、最新记录又不是 `stopped` 的 bot 单独重启，5 分钟冷却，30 分钟内 3 次仍起不来就停手只报一次。
...
这些操作只刷新桥和守护进程，已有 agent 会话通过持久状态继续复用。旧代码不认识新锁，首次升级须受控替换旧实例；该限制不能用重试超时掩盖。
...
受控 `start` / `stop` 给当前 handler 3 秒宽限，超时直接结束进程；强杀、断电恰好跨越不可查询的 TUI/斜杠副作用时，仍有无法自动判定的窗口，这是保留 uncertain 的明确边界（最坏是那一条需要重发，不会重复执行）。正常的桥重启不靠重复发任务解决它。入站未收到平台事件、磁盘损坏和平台撤回资源不属于本地收件确认保证。
...
- **永久逻辑异常不再静默**：仍保持 drainer 不崩、HWM 不推进，但按实际出错 record 写一次 `bot/offset/kind/error_type/error`；未知异常只留 digest，不落正文或 secret。同一错误签名在 HWM 未变化时去重，防毒记录每轮刷日志。
- **`_send_plain` 返回送达布尔**（True=发出 / False=失败），drainer 据此判要不要重试。**progress** 仍 best-effort（临时进度·可丢，不 hold 队列）。
- **验**：网络场景继续覆盖断网恢复补发、在线恰一次与恢复后不重复；逻辑异常另用临时 outbox 注入连续两轮同一 poison，证明 HWM 前进 `0B`、实际 kind 正确、未知错误不泄正文且日志回调只触发一次。

---

### § 2.5.3 · DM 目标怎么定 · 以及【降级投递必须留痕｜⚠️ 降级投递已于 2026-08-30 拆除】（2026-08-02 根治「新 bot 把 peer 当主人 → 刷群」）

> 🚩 **读之前先知道（2026-08-30）：本节描述的「降级投递」已经不存在了。** 兜底通道整条拆除
> （`_webhook_fallback` / `notify.py` / 看门狗 `notify_webhook` 全部删除），现在 DM 投不出去就是
> `delivered=false`／`via=null`，不会再改投任何地方。下面保留的是**事故史与当时的判断**——
> 「DM 目标怎么定」那部分仍然有效，「降级怎么留痕」已被「未送达怎么留痕」取代。
...
- **trace id 贯穿全链路**：每条消息分一个 6 位短码，一条消息从进到出按顺序打这几行——
  ...
  | `drainer logic error offset=… kind=… <type>: <digest>` | outbox 队头逻辑异常；HWM 未推进，同一签名只记一次 |
