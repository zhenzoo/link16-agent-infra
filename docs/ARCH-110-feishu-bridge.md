...
连不上飞书（DNS、网络、握手失败）时桥不退出，按 10/20/40/60 秒原地重连直到成功，控制状态为 `reconnecting`；连续失败两次才用 REST 纯文字给主人发一条断开通知，恢复后再发一条，中间不重复。主循环其他未预期异常立刻 `os._exit(1)` 并记 `failed`，不再让 SDK 线程把进程吊成收不到消息的僵尸；看门狗 R4 每轮对本机名册里没有进程、最新记录又不是 `stopped` 的 bot 单独重启，5 分钟冷却，30 分钟内 3 次仍起不来就停手只报一次。
