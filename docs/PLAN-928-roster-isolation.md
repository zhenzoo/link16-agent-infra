...
## 4.5 · 补：同一族的另外两处静默失败（三方复核后追加）

三台机互相复核时又挖出两个**同族**问题——形状都是「输出看起来正常，取材或标签是错的」：

- **`bridge_cron._roster_bots()` 是第二个独立的名册加载器**（tb24 发现）：它不走 `load_bots()`
  那条 fail-closed 的路，两个名册都读不到就静默 `return set()`。
  **失败形态不对称**才是关键：桥挂了 = 消息不通，几分钟就被发现；**cron 静默不跑 = 该发生的事
  没发生、零信号**，而且和「任务被主人关成 OFF」这个正常状态从外部看**一模一样**。
  → 改成**警告一次**（不 fail closed：守护进程硬退会把别的正常任务一起停掉），文案同款可照做。
- **`whoami.py` 把主人的 open_id 报成 bot 自己的**（tb24 发现 · tb25 被它误导过）：
  `bridge-session-<bot>.json` 里那个字段**叫** `open_id`、语义却是 DM 对端。旧版把它填进
  `info["open_id"]` 打成「我的 open_id」，还因为 `info["open_id"] or oid` 的短路，
  把当场从 `bot/v3/info` 查回来的**真** id 丢掉了。
  → 拆成 `open_id`（本 app 视角·唯一权威）与 `owner_open_id`（DM 对端），分两行打印；
  查不到自己就留空——**宁可不显示，也别显示一个会误导下一个人的值**。
  落盘字段名暂不改（要动桥的状态格式 + 三台机存量文件，超出「不要复杂」的边界），在读取处正名。

**副产物**：修完 `whoami` 顺手拿到了 per-app 隔离的干净证据——同一个主人，在 tuf19 三只 bot 的
三个应用下是**三个不同 open_id**（`ou_xxxxxx2…` / `ou_xxxxxx9…` / `ou_xxxxxx10…`）；
而 bot 自己的 id（`ou_xxxxxx1…`）与名册登记值完全一致，因为名册那条正是注册时用它自己的应用查的。
