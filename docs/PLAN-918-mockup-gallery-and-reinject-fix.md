...
- **2026-07-22** · **追加 A5：全仓 mockup 总览首页**（主人 2026-07-22 22:38 提：「能不能有个页面列出所有仓库、每个仓库下的 mockup，卡片带预览图，按仓库分类」→ 当场 mockup 拍板「就是要这个效果」→ 建成）。
  - **永久地址** = 展厅项目的**生产分支 `main`** → `https://mockups-demo.pages.dev/`（原先 404 空着的根域名；分支子域 `<slug>.mockups-demo.pages.dev` 照旧不变）。
  - **新脚本** `~/.claude-personal/skills/align/scripts/publish_gallery.py`：数据**全部线上实拉、零手维护清单** —— ① CF API 列项目全部分支（= 每仓一个门牌，**天然含另一台机器发的**）② 逐个拉 `<slug>.mockups-demo.pages.dev/` 解析页面清单 ③ 逐页取 `<title>` 当卡片标题 ④ 渲染成按仓分组卡片墙 → 部署到 `--branch main`。**任一台机器跑一条命令即可重刷全站。**
  - **卡片预览 = 实时 `<iframe>` 嵌真页面缩小**（已验证各 mockup 页无 `X-Frame-Options`，可嵌）+ `IntersectionObserver` 滚到才加载。选它而非烤 PNG 截图的理由：**零流程、永远最新**；若日后手机嫌重再补截图（两者可共存）。
  - **自愈**：`_get` 3 次重试 + **拉不到就沿用当前线上总览页里的旧清单**（实证 `video-studio` 因一次网络抖动被刷成 0 个）→ 总览页只会越刷越全，绝不因抖动把某个仓刷没。
  - **首刷实测**：11 个仓 / 32 个 mockup / 3 个历史门牌（`mockups-notes`、`taoci-prof` 旧 slug + `phdtaoci-cvpreview` 是 CV Hub 应用预览非 mockup）收进折叠区。
  - **顺带修 `publish_mockup.py` 部署卡死**：本机走系统代理通常 15s 传完，但**代理偶发把上传拖死 >5min 零输出**；而剥掉代理时**直连 DNS 又可能解析不了 `api.cloudflare.com`**（实测 `nslookup` 全超时）→ 谁都不能单靠。修法：**先按现有环境试、失败再剥代理试，每次 4 分钟封顶快速失败**（原来一挂就 10 分钟）。两个脚本同修。
  - **2026-07-22 追加侧边栏目录**（主人当场提）：左侧**粘性侧边栏** = 仓库一行 + 它下面每个 mockup 一行，点任意一行**平滑跳到那张卡并高亮 1.6s**；滚动时目录里当前仓自动高亮（scroll-spy）；搜索框同时过滤目录和卡片墙；点历史门牌的条目会**先自动展开折叠区**再跳。**手机（≤860px）**侧边栏收成顶部「展开目录」按钮，跳完自动收起。全部长在 `publish_gallery.py` 的模板里 → 以后每次刷新都带。
