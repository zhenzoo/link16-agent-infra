...
3. 确认无误后加 `--apply` 执行。
4. 电子表格写回自动独立回读目标范围逐格比对。正文Markdown的`overwrite`／`append`另核对原生表格数量、行列和每格内容；追加时连同原有表格一起检查，避免把旧表误当新增表。
   **回读不通过就不算写成功**；失败保留原链接，不自动回滚或另建文档。

### 5.1 正文里的表格必须使用原生表格

用户需要按行列比较数据。Markdown是工具的输入格式，工具必须把它写成飞书原生table和cell块；正文里出现`甲 | 乙 | 丙`并不等于形成表格。

新文档仍用`feishu_bridge.py send --bot <name> --doc <path>`。底层复用docio的官方CLI写入能力，创建后按固定文档版本读回所有原生块，逐表核对数量、形状与每格文字。原链接使用`docio write`，补丁的`docx.command`选`overwrite`或`append`，`docx.format`为`markdown`；先dry-run再apply。含表格的其他局部替换暂未具备同等验收，工具在写前拒绝，改用已经支持的操作。

覆盖前先读取现有正文、评论及资源，确认本地稿包含应保留的内容；不要用覆盖方式修复带有人类新增内容而尚未合并的文档。表格核验只证明结构和文字，长列的手机显示、字号与实际可读性仍需对应客户端观察，不能用API核验冒充真机浏览。

2026年9月14日的故障来自旧发布器的24格全篇预算：大表被降成竖线普通文字，文档仍返回成功。RESEARCH-060源稿应有5表183格，线上仅2表20格；读取覆盖率显示“全读到”，因为它只数线上已有资源。现已取消该预算及降级链，新增“源稿预期→线上实际”的核验。写入或核验失败保留文档ID，禁止自动导入另一份副本，修复后重新验收。

回归入口：`python -m unittest discover -s tests -p test_feishu_docs_text_publish.py`及`test_docio_*.py`。反例包含大于24格、同篇多表、整表变普通文字、缺格、错值、行列变形、无序转换块池、分页不完整及核验期间版本变化。真实修复证据由VoiceOver的`runs/PLAN-310/reference-v1/*table-check-v2.json`保存；来源矩阵不因发布成功而获得研究结论的验收。
