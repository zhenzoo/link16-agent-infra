...
- **做法**：两个租户的共享 a2a 群各加一个自定义机器人（免审）。名册 agent 新增 `tenant_key`，tenant 新增 `webhook_env`；`send_feishu_msg --to-agent` 发现两边租户不同就改走收件方群的 webhook，原样投递文字 + 真 @ 标签 + 发件人戳，路由记 `a2a-webhook`；同时把同一段话不带 @ 抄一份到发件方自己租户的群，两个群都能看到完整往来，缺配置当场报错不猜。收件桥不改：自定义机器人的 @ 照样唤醒应用机器人。ARCH-140 §8。
- **验证**：新增 `tests/test_send_feishu_msg_cross_tenant.py`（同租户不变、未登记不猜、缺地址拒绝、原样投递带真 @ 标签、主流程走 webhook 并记账、本方群抄送及其失败不影响投递）；tb26 真机：企业群投信口 @ tb26-link16 4 秒注入，`--to-agent tb25-link16` 自动走个人群投信口成功；tb24-link16 收到 TB26 的信后 37 秒回信进企业群（手写 @ 没唤醒，改用本工具即可）。
...
- **改动**（bbec1f6）：`bridge_workline_stop` 在工作行回执正常后调用同一个送达检查；最早未发记录超过 120 秒就拦下这一轮（"Before ending this turn …"），让模型当场查清并送出。`session_work.hold_for_delivery` 保证同一轮只拦一次，不会死循环。Claude、Codex、Kimi 的 Stop 都走这个 hook。
- **验证**：新增 2 个测试（卡住拦一次、刚写的积压不拦），全仓 1059 passed。
- **升级**：拉取即生效，不用重启桥。

## v0.34.2 — 2026-09-26 · 同一会话几秒内进多条消息，回复不再被堵死；每轮开头报送达状态

- **问题**：09-26 16:25 主人追问和两条后台任务通知在 3 秒内进了 tb24-link16 同一会话，桥给每条开一个轮次（工作行闸 pending）。模型只能给最新一轮提交工作行，旧轮次被拒（"turn 已过期"）后永远 pending；它们在 outbox 里的记录一直不放行，drainer 停在原处，之后所有回复排队。16:25–21:05 共 4.7 小时、6 条回复没发出去，doctor 重启 drainer 3 次无效后只在日志报"需人工"。
- **修复**（1333529）：`session_work._release_superseded`——最新一轮提交工作行或 Stop 修复判失败时，同一会话里开始时间不晚于它、仍 pending 的旧轮次一起放行，沿用这一轮的标题，记录按原顺序发出；比它晚开始的轮次不动。已经卡住的机器，下一次任意提交就会自愈。
- **送达检查**（1333529）：`bridge_userprompt._delivery_context` 每轮开头只读 outbox 大小、HWM 和书签处记录的时间，注入一行 `Delivery check: ok` / `STUCK`（最早未发记录超过 120 秒才算卡住），卡住时写明积压多少、卡在几点、该查哪三个文件，要求智能体先查清并送出再继续。不依赖工作行开关，不写状态、不重试。
- **验证**：新增 6 个测试，全仓 1057 passed；tb24 当晚手工放行 7 个被取代的轮次后，21:05:49–21:06:11 补发 17 张卡，本机 23 个 bot 送达检查全部 ok。
- **升级**：拉取即生效，不用重启桥（改动都在每次新起进程的 CLI 和 hook 里）。
...
- **原因**：v0.33.0～v0.33.2 已让桥断网时一直重连、`start` 超时不再判失败；但 `_start_locked` 在「某只桥启动即退出」或「cron 起不来」时仍立即抛错，排在后面的看门狗照样不起——而看门狗正是唯一会单独复活死桥的部件。
- **修正**：失败先记下、桥/cron/看门狗全部尝试启动后再统一报错；返回码与报错文本不变，单 bot `start --bot` 语义不变。
- **升级注意**：开机任务必须指向 Python 3.12+ 的 `pythonw.exe`。tb24 的 `FeishuBridge-Autostart` 此前仍是 Miniconda 3.10（`hashlib.file_digest` 等 3.11+ 路径在开机拉起的桥上会崩），已用 `service_installer.py plan → apply` 改到 3.14，并把 wmux 开机项改为不带版本号的 `wmux\wmux.exe`。其他机器跑一次 `plan` 即可自查。
- **验证**：新增回归测试（桥启动即退出 + cron 失败时看门狗仍被拉起、最后报两条错），旧代码失败、新代码通过；全仓 1024 passed（另 4 个 miaoda 测试仅在 Python 3.10 失败，3.14 下通过）。
...
- **原因**：消息走已建立的长连接，不需要 DNS；点 👍 是单独一次 HTTP 调用，要先解析域名。旧代码失败直接吞掉，不重试、不记日志，返回 `success=False` 也当成功。
- **修正**：`thumbs_up` 在后台按 0/2/5/15/30/60 秒重试（约 2 分钟），不阻塞消息处理；`success=False` 视为失败；最终失败记一行日志。
- **验证**：新增 DNS 抖动后第 3 次点上、全部失败只记一次日志两项回归；全仓 1027 passed。

## v0.33.2 — 2026-09-24 · 断网提醒全机只发一次

- **不再按 bot 刷屏**：v0.33.1 每只 bot 各自提醒，整机断网时 10 只 bot 会各发“断开”“恢复”共约 20 条。现在 `claim_outage_alert` 用 O_EXCL 锁文件做全机单发：30 分钟内只有第一个抢到的桥发一条断开通知，恢复后由它再发一条；重连过程本身不发任何消息。日志里短暂断开按秒显示。
- **断网时 start 不再判失败**：桥会一直等 SDK 重连，`start` 超过验收时限只提示“还没连上飞书，进程在自动重连”，并照常拉起看门狗和 cron（以前会抛错、看门狗和 cron 起不来）。
- **验证**：新增 10 只 bot 同时断网只发一对提醒、30 分钟内再断不提醒、过窗口可再提醒；全仓 1025 passed。
...
- **stop / start 立即生效**：去掉“正在完成当前消息交接”最多 180 秒且不强杀的等待。空闲桥 0.5 秒内自行退出，3 秒宽限后直接结束；启动中、重连中的桥直接结束。收件箱落盘保证不丢消息；停止后记 `stopped`。
- **看门狗 R4 单 bot 重启**：以前只在全部桥都挂时告警一次，单只 bot 挂掉无人察觉。现在对没有进程且最新记录不是 `stopped` 的 bot 单独重启并 DM 一次；5 分钟冷却，30 分钟 3 次仍失败就停手只报一次。
- **同期**：76cf3cb 移除反复发送的启动进度卡，会话创建失败只告警一次、原消息保留不自动重放。
- **验证**：新增 stop 宽限/立即结束、重连成功与单次抖动静默、R4 只重启崩溃 bot 与停手上限等回归；全仓测试通过。
...
- `rename_bot.py` 提供 plan、后台链接、线上身份核对、apply、verify、后台 watch、cancel 和 rollback；两份名册只更新显示名、@ 名及历史别名，拒绝重名、过期计划和错配凭据。
- `bot_names.py` 统一名称解析；常用命令先将新名或别名转换为内部代号，再读取状态或检查发送权限。旧名继续可用，身份与重复投递限制不因别名放宽。
- SOP-125 收拢人工改名、本地同步和中断恢复，并用 5 号智能体的真实八步案例解释每一步查哪里、改哪里；应用编号、后台地址示意和密钥在公开文档中脱敏。
- 规范历史安装文档路径及测试中的主机名、用户目录占位值，使公开仓脱敏检查恢复通过；检查器与白名单未改。
...
- **后台回传可诊断**：注册 HTTP 连接/读取超时为 10/20 秒，临时 poll 故障最多尝试三次，保留同一授权任务；不自动重放创建步骤。Windows 探活只查询进程，重复启动复用活注册器，失败事件与脱敏诊断可回传。
- **完成信号可信**：SDK 真正回传且本机登记成功后才记 registered；权限审阅事件原子排队，旧探测结果不能覆盖失败。个人 core 不再误报已入群；客户端智能体标签仍需独立确认。
- **业务目录可定位 Link16**：公共 launcher、飞书 worker 与 Codex app-server 子进程显式传递根路径；SOP 分清路径缺失、在线策略关闭与 API 缺权。登记 tb25-media-meta 的公开身份，凭据和业务 cwd 留在本机配置。
- **验证**：在 v0.23.0 后最新 main 上合并验证，注册 41、公共启动器 62、Kimi 24、Codex app-server 11、wmux worker 16、交付策略 5，共 159 项测试通过。两处新增位置冲突均保留双方内容；Codex 启动信任检查的另一项未提交工作不属于本版本。
- **同期文档**：保留 Kimi 合并验收回填与公开安装基线计划；该计划未改变仓库公开范围，也不代表安装改造已经完成。
- **升级边界**：下次注册与新启动器执行使用更新代码；不因 Git 更新自动重启常驻桥。需刷新现有服务时仍遵守维护窗口。

## v0.23.0 — Kimi Code 原生接入（2026-09-06）

此前 Kimi 接入仅在 `feat/kimi-native-profile`，其他电脑更新 `main` 无法取得。此版本将完整接入合入默认分支，Link16 可通过飞书驱动 Claude Code、Codex 和原生 Kimi Code。

### 新增

- **原生 Kimi profile**：registry 支持 `runtime=kimi`，使用隔离的 `KIMI_CODE_HOME`；已注册账号可从终端启动或通过飞书 `/account <profile>` 显式切换。它与旧 `cck`（Claude 连接第三方模型）分开，不复制其他账号认证。
- **真实会话与进度回传**：原生 TUI 执行工具，独立观察程序读取该会话的 Wire 1.5 主 agent 日志，复用现有路由、计划卡、outbox 和送达回执。支持公开进度、真实 todo 状态、最终答案、取消、失败和同会话恢复。
- **可恢复与去重**：观察程序可独立重启，保留原生终端；会话与目录精确绑定，历史重放只重建状态，不重复发旧答案。最终正文仅进入答案卡；隐藏思考、工具原始参数及结果留在本机。

### 同期修复与验证

- 合入 main 的模型与参数修复：Codex seed 不再固定模型/推理强度，保留原生选择；从用户 home 启动隔离账号时避免加载另一个账号的配置；终端参数、短参数和 slash 参数按原值传递。Codex wmux worker 可显式指定本次模型、effort 和 Fast，省略时沿用原生默认。
- 保留 v0.22.1/v0.22.2 的进程观测、互斥和 cron 未知状态修复。既有三机 53 桥部署证据属于原有 runtime 基线，不代表这 53 只 bot 已切换 Kimi。
- 合并后回归：**598 tests、56 subtests 通过**。初始隔离验收使用原生 CLI **0.38.0 / Wire 1.5**；2026-09-06 本机已升级为 **0.41.0**，真人私聊对应 journal 仍为 Wire 1.5，入站、Kimi 回复、工具进度与最终答案均已在本机收发历史及送达回执中对齐，用户确认接通。
- **沿用 terminal 的模型选择**：Link16 不注入 Kimi 模型或思考强度；新会话使用所选 profile 的原生默认配置，恢复会话由 Kimi 保留自身设置。本机 23:19:57 切换模型后，两次实际请求均为 `k3 / high`，与该 profile 保存值一致；不宣称在另一个终端改默认会热更新已运行会话。

### 升级、限制与回滚

- 各机拉取本仓 `main` 后，仍需自行安装并登录原生 Kimi、在本机 effective registry 登记隔离 profile，再做 doctor、自测与真人 DM 往返。升级常驻桥需要维护窗口；只更新文件不等于运行中服务已加载。
- 个人 Stage／ETA 规则属于各自的配置仓，需要另行同步并渲染受管入口、新会话加载；Link16 不分发维护者的私人配置或凭据。
- Kimi 原生交互选择器尚未映射为飞书按钮；额度保持 unknown，自动选号不选择 Kimi。Kimi 群聊真机验收与跨机部署未由本次本机 DM 验收替代。升级 Kimi 协议后须重新验证，不承诺未知 Wire 版本兼容。
- 回滚可在维护窗口将 Kimi bot 切回原 profile，并只重启相关桥；保留 Kimi home、会话与投递记录。合并前 main 基线为 `170d513`；不要将不支持 Kimi 的旧代码与仍含 `runtime=kimi` 的 registry 混用。

机制见 [ARCH-120 §11](docs/ARCH-120-agent-profile-runtime.md#11-原生-kimi-code原生终端与独立-wire-观察程序)，本次合并与分支审计见 [PLAN-1070](docs/PLAN-1070-kimi-main-release.md)。本版本为 private 仓内发布节点，尚未改变仓库公开范围。

## v0.22.2 — 补齐 cron 看板和菜单的未知状态提示（2026-09-06）

- 审计剩余调用方发现 cron 看板、菜单仍按真假值把查询未知显示为没跑；看板现在非零退出，菜单显示未知且不提示启动。保留已保存任务配置，未知不等于任务关闭。

## v0.22.1 — 看门狗巡检恢复与桥进程控制纠错（2026-09-06）

- 修复看门狗 R6 水位巡检和 R4 桥死亡通知的未绑定变量；发送失败不再消耗已告警状态或冷却时间，心跳增加逐项检查结果。
- 桥、cron、watchdog 使用统一 Windows 查询：15/45 秒窗口重试，严格校验成功标记、错误输出和进程列表；未知不再显示为没有运行，启停失败返回非零。
- 服务持有操作系统文件锁到退出，控制命令另行串行化；`run` 不再互相顶替，`start` 确认旧进程退出后替换。整体 `stop` 先查询，桥停止失败保留守护进程。
- 升级需要刷新桥、cron、watchdog，保留 agent 会话。锁范围是同一 Windows 用户；不声称跨用户或旧版本服务具备新锁保护。执行和跨机验收见 [PLAN-1060](docs/PLAN-1060-bridge-process-health-and-rollout.md)。
...
- 速记员席位用文件锁做硬闸：同一只 bot 同一时刻只允许一个观察者，**拿不到席位就拒绝启动**（还没连 app-server、还没写过一行就退），杜绝两个观察者把同一条 final 写两遍 outbox；锁由操作系统持有，进程崩了席位自动腾出。worker 端监工在子进程非预期退出时把它拉回来——**杀掉它就是升级**，子进程重新从磁盘读代码，于是回程可热更新。
- 观察者断线不再永久死：退避重连并重新 resume 回同一条 thread；两分钟仍挂不回去就借 outbox 告警（那条路与本连接互相独立，断线时照样送得出去），接回来再报平安。单条事件解析出错也不再让整个观察者线程静默死掉。
- 修正 2026-08-29「消息进得去、回复出不来、静默 8 小时」事故的真根因：到本机 app-server 的 websocket 因用户级 `HTTPS_PROXY` 被第三方代理进程转发——loopback 不会无缘无故断，闲置三天的代理隧道会。`RpcConnection` 显式 `proxy=None` 直连。
- 桥的投递保证补上时间闸：`agentStatus` 只能证明会话没死、证明不了回程还通，回程静默超过 30 分钟升级喊人一次。判据取 outbox 最后写入时间而非 pending 注入时间——定时任务每 5 分钟注一条会把后者不断刷新，用它做判据永远够不到阈值，而事故恰好是这个形状。
- `codex exec` 不再被 launcher 一把捅穿沙箱：交互式 worker 仍拿全套 bypass flag，非交互 exec 一个都不给，调用方立的权限闸不再被背后作废。Claude 的 Stop hook 补上黑匣子留痕，「没跑」与「跑了但死在门口」不再长得一样。

## v0.19.0 — 出站止损与全进程陈旧检测（2026-08-31）

- outbox 水位书签改为 fail-closed 并在落盘时 `fsync`：书签损坏时退到 outbox 末尾并留痕，不再把全量历史误判为未发送；看门狗新增 R6，能主动报告水位书签损坏。
- 完整拆除已经失效的群机器人兜底通道：DM 无目标或发送失败会在 receipt 明确记录失败并有界重试，绝不再改投群聊；同步清理桥、看门狗、工具索引与注册文档里的陈旧降级语义。
- 看门狗新增 R5，能识别 Codex 回合被服务端中断；陈旧代码检测从“只盯看门狗自己”扩展到全部常驻进程，并由真实 CommandLine、入口 Python 文件及其模块级本仓 import 动态反推监控范围，不再维护会腐坏的进程类型清单。
- doctor 自愈重启增加熔断，避免打断 drainer 自带的有界重试；注册 SOP 固化 OAuth、权限审批、私聊认主等人工停点的机械监视信号，并明确新增 bot 只单独启动该实例。
- 新机文档补齐 OpenSSH 只属于可选 `envsync` 的边界与跨机 a2a 真判据；舰队 registry 纳入新增 tb25 bot，套磁三、四号的旧循环以明确停用状态进入版本控制。
